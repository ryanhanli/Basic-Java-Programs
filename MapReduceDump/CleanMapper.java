...
public class CleanMapper
  ...
  public void map(LongWritable key, Text value, Context context)
	...
	if (songs.contains(ID)){
		String acousticness = values[0];
		String artists = values[1];
		String danceability = values[2];
		String energy = values[4];
		String explicit = values[5];
		String songName = values[12];
		String year = values[18]; 
		String instrumentalness = values[7];
		String loudness = values[10];
		String popularity = values[13];
		String release_date = values[14];
		String speechiness = values[15];
		String tempo = values[16];
		String valence = values[17];
		//String year = values[18];
		
		String combined = acousticness + " " + artists + " " + danceability + " " +
							energy + " " + explicit + " " + ID + " " + instrumentalness + " " +
							loudness + " " + songName + " " + popularity + " " + release_date + " " + speechiness +
							" " + tempo + " " + valence + " " + year;
							
		context.write(new Text(combined), new IntWritable(1));
	}
  }
}
